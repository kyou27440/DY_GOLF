...
### v1.3.3 — 2026-08-27 (PC ↔ 모바일 실시간 클라우드 동기화 핫픽스)

| 순번 | 변경 내용 | 변경자 |
|---|---|---|
| 012 | **등수별 금액/회비 산출 시트 Supabase 클라우드 동기화 구축**: `app_settings` 테이블 기반 실시간 듀얼 백업/동기화 적용 | AI |
| 013 | **`Store.syncAllLocalDataToSupabase()` 동기화 차단 버그 수정**: `supabaseClient` 객체 유효성 체크 로직 픽스 | AI |
| 014 | **게임 기록 탭 등수별 지불금 및 연동 배지 기기간 공유 보장**: 로컬 캐시와 클라우드 데이터 병합 무결성 강화 | AI |
| 015 | **모바일 브라우저 PWA 캐시 버스팅 적용**: `v1.3.3_20260827`로 버전 업데이트 | AI |
